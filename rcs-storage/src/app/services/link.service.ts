...
export class LinkService { 
  ...
  constructor( 
    ...
    @Inject(DOCUMENT) private document 
      ...
      this.renderer = this.rendererFactory.createRenderer(this.document, { id: '-1', encapsulation: ViewEncapsulation.None, styles: [], data: {} }); 
    } 
    
  updateTag(tag: LinkDefinition) { 
    this.removeTag(tag); this.addTag(tag); 
  ...
  removeTag(tag: LinkDefinition) { 
    try { 
      ...
      } /** * Inietta il link ocme ultimo child del tag */ 
...
export declare type LinkDefinition = { 
  charset?: string; crossorigin?: string; href?: string; hreflang?: string; media?: string; rel?: string; rev?: string; sizes?: string; target?: string; type?: string;
 } & { [prop: string]: string; };
