export const  homePageTitle = ``;
export const homePageContent = ``;


export const viewRatesPageTitle = `View Rates | Storage World`;
export const viewRatesPageContent = `View our unit rates and start your reservation process right here!
Have questions? Call our on-premises manager for help during business hours!`;


export const reservePageTitle = ``;
export const reservePageContent = ``;

export const payRentPageTitle = `Pay Rent Online | Storage World`;
export const payRentPageContent = `Pay your rent online 24/7/365! Follow the instructions here or
call our office to learn how to pay at your convenience!`;


export const storageTipsTitle = ``;
export const storageTipsContent = ``;


export const unitSizerPageTitle = ``;
export const unitSizerPageContent = ``;


export const contactPageTitle = ``;
export const contactPageContent = ``;
