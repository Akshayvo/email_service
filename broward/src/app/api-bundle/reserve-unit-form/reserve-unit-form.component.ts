    ...
    this.getDataSubscribe$ = this.fetchDataService
      ...
      .subscribe((unitTypesResponse) => {
        if (this.dataSharingService.LstUnitTypes) {
          const index = unitTypesResponse.lstUnitTypes.findIndex(
            (x) =>
              x.UnitTypeID === this.dataSharingService.LstUnitTypes.UnitTypeID
          );
          this.lstUnitTypes = unitTypesResponse.lstUnitTypes;
          this.getFilterLstUnitTypes(unitTypesResponse);
          if (index >= 0) {
            const defaultMonthlyValue =
              unitTypesResponse.lstUnitTypes[index].MonthlyRate;
            this.description =
              unitTypesResponse.lstUnitTypes[index].Description;
            this.reservationFeeTax =
              unitTypesResponse.lstUnitTypes[index].ReservationFeeTax;
            this.reservationFee =
              unitTypesResponse.lstUnitTypes[index].ReservationFee;
            this.MoveIn.intUnitTypeID =
              this.unitTypeID ||
              unitTypesResponse.lstUnitTypes[index].UnitTypeID;
            this.unitTypeId =
          ...
          if (!serviceDescriptionValue && !serviceMonthlyValue) {
            ...
            this.dataSharingService.LstUnitTypes.MonthlyRate = this.monthlyRate;
            this.dataSharingService.LstUnitTypes.AnnualRate = this.annualRate;
            this.dataSharingService.LstUnitTypes.BiAnnualRate =
              this.biAnnualRate;
            this.dataSharingService.LstUnitTypes.QuarterRate = this.quarterRate;
          } else {
            this.reserveUnitForm.patchValue({
    ...
    this.getRentalPeriodSubscribe$ = this.fetchDataService
      ...
      .subscribe((rentalPeriodResponse) => {
        ...
        if (this.LstRentalPeriods.length != 0) {
          ...
          this.dataSharingService.periodID =
            rentalPeriodResponse.lstRentalPeriods[0].PeriodID;
          this.reserveUnitForm.patchValue({
            lstRentalPeriods: [
              {
