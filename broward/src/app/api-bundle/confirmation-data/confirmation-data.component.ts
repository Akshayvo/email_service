...
export class ConfirmationDataComponent implements OnInit, OnDestroy {
  ...
  fetchSharedData() {
    ...
    console.log(
      "this.dataSharingService.objTenant",
      this.dataSharingService.objTenant
    );
  ...
  public fetchOption() {
    ...
    if (!!this.dataSharingService.objTenant.State) {
      this.index = JSON.stringify(
        this.options.findIndex(
          (x) => x.id === this.dataSharingService.objTenant.State
        )
      );
      if (!!this.options) {
    ...
    if (this.dataSharingService.navigateToMoveIn) {
      ...
      if (this.dataSharingService.navigateToReserve) {
        this.router.navigate([
          `${environment.locationName}/view-rates/reserve`,
        ]);
      }
    }
  }
  ...
  getTenantUnitData() {
    ...
    this.commandingOfficer =
      this.dataSharingService.objTenant.CommandingOfficer;
    this.commandingOfficerPhone =
      this.dataSharingService.objTenant.CommandingOfficerPhone;
  }
  ...
  updateTenant(data: any) {
    this.updateTenantSubscribe$ = this.tenantInfoService
      .updateTenant(data)
      .subscribe((result) => {
        if (result.intErrorCode === 1) {
          if (this.navigateToMoveIn) {
            if (this.dataSharingService.MoveInData.TotalChargesAmount > 0) {
              this.router.navigate([
                `${environment.locationName}/view-rates/payMoveInCharges`,
              ]);
            } else {
              this.moveIn(this.MoveIn);
            }
            ...
            if (this.navigateToReserve) {
              // console.log(this.dataSharingService.LstUnitTypes.ReservationFee);
              if (this.dataSharingService.LstUnitTypes.ReservationFee > 0) {
                this.router.navigate([
                  `${environment.locationName}/view-rates/payReservationCharges`,
                ]);
              } else {
                this.makeAReservation(this.MoveIn);
              }
  ...
  onSubmit() {
    ...
    this.reservationInProgress = true;
    //console.log(this.existingTenantToken);
    if (this.existingTenantToken) {
      ...
      if (this.existTempToken) {
        ...
        if (this.navigateToMoveIn === true) {
          ...
          //console.log(this.dataSharingService.getUnitData().ReservationFee);
          if (this.dataSharingService.LstUnitTypes.ReservationFee > 0) {
            this.dataSharingService.addingTenant = true;
            this.dataSharingService.addingTenant = true;
            this.router.navigate([
